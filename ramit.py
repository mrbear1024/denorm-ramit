# RAMiT (Reciprocal Attention Mixing Transformer) 文件实现 RAMiT 模型

import torch.nn as nn # 导入 PyTorch 的模块
from einops import rearrange # 导入张量重排工具
import torch.nn.functional as F # 导入函数式 API
import torch # 导入 PyTorch
from torchvision.transforms import functional as TF # 导入图像变换函数 （pad/normalize）
import math  # 数学库
try:
    from .common.mean_std import mean_std # package import
except ImportError:
    from common.mean_std import mean_std # fallback for direct script run
from utils.etc_utils import denormalize # 导入反归一化函数
from timm.models.fx_features import register_notrace_function # 装饰器
from timm.models.layers import trunc_normal_, to_2tuple # timm 权重初始化与工具函数

def make_model(args, opts, pe):  # 构建模型的工厂函数。
    model = RAMiT(target_mode=args.target_mode,  # 任务模式（SR/去噪等）。
                  img_norm=args.img_norm,        # 是否做归一化。
                  in_chans=opts['in_chans'],     # 输入通道
                  dim = opts['dim'],            # 基础通道数。
                  depths = opts['depths'],      # 各 stage 深度。
                  num_heads = opts['num_heads'], # 注意力头数。
                  head_dim = opts['head_dim'],   # 每头维度。
                  chsa_head_ratio = opts['chsa_head_ratio'],  # 通道注意力头比例。
                  window_size = opts['window_size'],     #  窗口大小。
                  hidden_ratio = opts['hidden_ratio'],   # FFN 扩展倍数。
                  qkv_bias = opts['qkv_bias'],      #  QKV 是否带 bias。
                  mv_ver = opts['mv_ver'],          # Mobile Variants 版本
                  exp_factor=opts['exp_factor'],    # 扩展因子
                  expand_groups=opts['expand_groups'], # 分组扩展数
                  act_layer = opts['act_layer'],   # 激活函数类
                  norm_layer = opts['norm_layer'],  # 归一化层
                  tail_mv = opts['tail_mv'],     # 尾部 mv 层数
                  attn_drop = opts['attn_drop'], # 注意力 dropout
                  proj_drop = opts['proj_drop'], # 投影 dropout
                  drop_path = opts['drop_path'], # 随机深度比例
                  helper = opts['helper'],       # 辅助分支开关。
                  mv_act = opts['mv_act']        # mv 激活函数
                 ) # 创建 RAMiT 实例，参数来自 args/opts。
    num_params = sum([p.numel() for n,p in model.named_parameters()]) #统计参数量
    print(format(num_params, ',')) # 打印参数量
    return model
...
class QKVProjection(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=True):
        ...
        self.qkv = nn.Conv2d(dim, 3*dim, 1, bias=qkv_bias)        #attention 中，qkv通常来自于一个卷积或者线性层
        
    def forward(self, x):
        B, C, H, W = x.size()
        qkv = self.qkv(x)
        qkv = rearrange(qkv, 'b (l c) h w -> b l c h w', l=self.num_head) #Head Split 把“混在一起的多头通道维”，拆成“显示的head维度”，只做了升维，不改变数值，只改变张量结构。为多头注意力的并行计算做结构准备，每个head是一个独立维度，独立做attention。
        return qkv
...
class SpatialSelfAttention(nn.Module):
    def __init__(self, dim, num_head, total_head, window_size=8, shift=0, attn_drop=0.0, proj_drop=0.0, helper=True):
        ...
        ######可学习的参数，初始化为ln(10)，以对数形式存储。
        #为什么存对数，因为对数空间对计算机更友好。原本数值如果是 0.0001 到 10000 这样跨度巨大的范围，在对数空间里可能只是 -9 到 9。这能让模型在训练更新参数时更稳定，不容易出现梯度爆炸。
    ...
    def forward(self, qkv, ch=None):
        ...
        # 余弦相似度是通过向量归一化后的点积实现的。
        logit_scale = torch.clamp(self.logit_scale, max=math.log(1. / 0.01)).exp() 
        ##### 功能：计算一个缩放因子，用于调整Qurey和Key之间余弦相似度的量级。 
        #torch.clamp截断操作，将该参数最大值限制在ln(100),防止缩放因子的指数值过大。如果缩放因子的太大，Softmax的输出会变得极度尖锐（类似One-hot）,导致梯度消失、模型无法训练。
        # 为什么要用.exp()变回来。在计算注意力矩阵时，需要的是一个正数乘法因子。执行.exp()操作后，无论原本的 $x$ 是正还是负，结果一定是一个大于 $0$ 的“普通数值”。
        attn = attn * logit_scale
        #logit_scale得到的这个数值看作是注意力机制的“对比度旋钮”，如果数值很大，会极大地拉开像素之间的差距，让“像”的地方极亮，“不像”的地方极黑。如果数值很小，像素之间的差距很小，大家都分得差不多。
...
class NoLayer(nn.Identity):                              #这个类的功能
    def __init__(self):
...
class DRAMiTransformer(nn.Module): # Reciprocal Attention Transformer Block
    ...
    def forward(self, x, sp_=None, ch_=None):
        ...
        attn = self.drop_path(self.norm1(attn0)) + x # LN, skip connection [B, C, H, W]                #drop_path做什么功能的？
...
class EncoderStage(nn.Module):
    def __init__(self, depth, dim, num_head, chsa_head_ratio, window_size=8, head_dim=None, 
        ...
        self.num_head = num_head
        self.window_size = window_size
        shift = window_size//2       #“窗口平移”大小，在许多窗口注意力/局部块操作里，会把窗口平移半个窗口大小，用于让相邻窗口之间有交互，从而扩大感受野。
        
        self.blocks = nn.ModuleList()      #执行完后，self.blocks 会变成一个空的 nn.ModuleList 容器，里面还没有任何子模块。后续你需要往里 append 模块，否则它不会包含层或参数。
        for d in range(depth):
            self.blocks.add_module(f'block{d}', DRAMiTransformer(dim, num_head, chsa_head_ratio,
                                                                 ...
                                                                 attn_drop, proj_drop, drop_path, helper, mv_act))#“交替平移窗口”策略，“偶数层不平移、奇数层平移”的交替策略
    def forward(self, x):
        sp, ch = None, None
        for i, blk in enumerate(self.blocks):
            x, sp, ch, attn = blk(x, sp, ch)
            sp, ch = (None, None) if (sp.size(1)==0 or ch.size(1)==0) else (sp, ch) # pure SPSA or CHSA            #没看懂
        return x, attn
...
class Bottleneck(nn.Module):
    ...
    def shallow_down(self, x, r):
        for _ in range(r):
            x = F.max_pool2d(x, 2)      #下采样操作。把每个 2x2 区域取最大值，空间尺寸减半（H、W 各 /2），通道数不变。         
    ...
    def forward(self, x_list):
        ...
        for i in range(len(x_list[1:])):
            ...
            new_x.append(F.pixel_shuffle(x_, 2**i))             #上采样是把通道搬到空间，通道数会缩小r^2倍，空间变大r倍。
...
class Reconstruction(nn.Module):
    def __init__(self, target_mode, dim, kernel_size=3, stride=1, num_mv=2, mv_ver=1, mv_act=nn.LeakyReLU, exp_factor=1.2, expand_groups=4):
        ...
        self.kernel_size = kernel_size
        self.stride = stride
        self.num_mv = num_mv #mobivari的层数？
        
        self.mobivari = nn.ModuleList()
        for i in range(num_mv):     # num_mv=2 ModuleList容器内Movivari数量
            if mv_ver==1:
        ...
        self.shuffle = nn.PixelShuffle(upscale) if upscale!=1 else nn.Identity() #nn.Identity() 是一个“什么都不做”的层：输入是什么就原样返回。常用来在需要“可选模块”的地方占位，比如满足接口，但不改变数据。
        self.final_conv = nn.Conv2d(self.out_chans, self.out_chans, kernel_size, stride, kernel_size//2)
        
    def forward(self, x):
        for mobivari in self.mobivari:    #过了2层Mobilvari
            x = mobivari(x)
...
class RAMiT(nn.Module):
    def __init__(self, in_chans=3, dim=64, depths=(6,4,4,6), num_heads=(4,4,4,4), head_dim=None, chsa_head_ratio=0.25,
                 ...
                 act_layer=nn.GELU, norm_layer=ReshapeLayerNorm, tail_mv=2, weight_init='', target_mode='light_graydn', img_norm=False,
                 attn_drop=0.0, proj_drop=0.0, drop_path=0.0, helper=True, mv_act=nn.LeakyReLU, **kwargs):
                 # 输入通道数是3（RGB图像常为3）。主干特征通道宽度是64。本次主要研究采用的D-RAMiT Block数量设置是6、4、4、6。每个stage的heads数量是4， 4， 4， 4。head_dim若为空，通常数值为dim/num_heads。通道自注意力机制的比率0.25。窗口注意力的窗口尺寸window_size = 8。
        super(RAMiT, self).__init__()
        self.unit = 2**(len(depths)-2)*window_size                      #计算模型处理图像时所需的最小对齐单元，确保模型在最底层进行自注意力计算的窗口大小
        ...
        self.scale = int(target_mode[-1]) if target_mode in ['light_x2', 'light_x3', 'light_x4'] else 1      #缩放倍率用来判断任务类型（是否超分），从而决定是否保留低质输入。
    ...
    def forward_size_norm(self, x):                                             #核心功能：为模型输入进行尺寸对齐（填充）和数值标准化，具体来说确保输入图像长宽能够被模型处理的最小单元整除，并符合模型训练分布。
        _,_,h,w = x.size()
        padh = self.unit-(h%self.unit) if h%self.unit!=0 else 0                 #尺寸检查与像素填充：输入图像的长和宽必须是窗口大小的整数倍
        padw = self.unit-(w%self.unit) if w%self.unit!=0 else 0
        x = TF.pad(x, (0,0,padw,padh))                                          #后向填充      左、上、右、下
        return TF.normalize(x, self.mean, self.std) if self.img_norm else x     #通常被称为 Z-Score 标准化.在图像处理中，像素的均值代表了图像的整体亮度。减去均值后，模型处理的是像素相对于亮度的相对变化。这有助于模型学习图像的纹理和结构，而不是被图片的明暗程度带偏。标准差代表了图像的对比度。除以标准差后，无论原始图片是高对比度还是低对比度，输入到模型里的“信号强度”都是一致的。这保证了网络中的权重更新不会因为某张图对比度特别强而产生过大的梯度。
    
    def forward(self, x):
        _,_,H_ori,W_ori = x.size()
        x = self.forward_size_norm(x)
        B, C, H, W = x.size()
        x_lq = x if self.scale == 1 else None # due to residual image (except for only SR) #超分任务不需要做残差，其余需要做残差。
        ...
        out = denormalize(out, self.mean, self.std) if self.img_norm else out    #把输出从“标准化后的范围”还原回原始像素分布
        out = out[:, :, :H_ori*self.scale, :W_ori*self.scale]                     #把输出裁剪回原始尺寸（或超分后的尺寸）
        return out
    ...
    def _init_weights(self, m):
        # Swin V2 manner + basic Conv/BN init for stable training
...
if __name__ == '__main__':
    model = RAMiT(in_chans = 1, dim = 64, target_mode = 'light_graydn')
